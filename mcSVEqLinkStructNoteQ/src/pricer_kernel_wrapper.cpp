    ...
    double& tk)
{
    ...
    std::vector<int, aligned_allocator<int>> c_data(miMax + 1);
    std::vector<int, aligned_allocator<int>> l_data(miMax + 1);
    std::vector<int, aligned_allocator<int>> r_data(miMax + 1);
    std::vector<real_t, aligned_allocator<real_t>> qasave(miMax + 1);
    std::vector<real_t, aligned_allocator<real_t>> qbsave(miMax + 1);
    ...
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_dirnum));
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_c_data));
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_l_data));
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_r_data));
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_qasave));
    OCL_CHECK(
        err, err = krnl_mcSVEqLinkStructNote1.setArg(arg_id++, buffer_qbsave));
    OCL_CHECK(
